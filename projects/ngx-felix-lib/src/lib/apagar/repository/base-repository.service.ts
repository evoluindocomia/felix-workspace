// projects/ngx-felix-lib/src/lib/repository/base-repository.service.ts
...
export abstract class BaseRepository<T extends IResource> implements IRepository<T> {
  ...
  // Injeções automáticas via propriedades (sem constructor)
  protected http = inject(HttpClient);
  protected config = inject(API_CONFIG);
  protected contextService = inject(EnterpriseContextService);

  /**
   * Resolução inteligente de URL:
   * 1. Prioriza o Contexto Dinâmico (Recebido do MFE Host)
   * 2. Fallback para o Environment (Para desenvolvimento local/isolado)
   */
    ...
    if (!baseUrl) {
       throw new Error('[ngx-felix-lib] URL da API não resolvida no Contexto nem no Environment.');
    }
