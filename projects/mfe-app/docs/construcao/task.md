...
Esta lista de tarefas guiará o desenvolvimento das aplicações `root-app` e `mfe-app` conforme definido no PRD.
...
- [x] **Fase 2: Bootstrapping das Aplicações (Independência)**
  ...
  - [ ] Criar projeto Angular MFE: `ng new mfe-app --no-create-application --directory projects/mfe-app`.
  - [ ] Criar aplicação nfe interna: `ng generate application mfe-app`.
...
- [ ] **Fase 3: Configuração de Module Federation**
  ...
  - [ ] Adicionar `@angular-architects/module-federation` ao `mfe-app`.
  - [ ] Configurar o webpack/federation config do `mfe-app` para expor o componente remoto.

- [ ] **Fase 4: Integração de Dependências**
  - [ ] Instalar `ngx-felix-lib`, `crypto-js` e `@types/crypto-js` no `root-app`.
  - [ ] Instalar `ngx-felix-lib`, `crypto-js` e `@types/crypto-js` no `mfe-app`.
...
- [ ] **Fase 6: Implementação do Remoto (`mfe-app`)**
  ...
  - [ ] Adicionar o Interceptor corporativo no `app.config.ts` do `mfe-app`.
...
- [ ] **Fase 7: Testes e Validação Final**
  - [ ] Executar o `mfe-app` em uma porta específica (ex: 4201).
